...
The CMV config defines how different views are coordinated.
Ultimately, this configuration must be a JSON object when it is passed to the Provider component `config` prop.
...
`CmvConfig` is a class representing a CMV config.
To begin creating a config with the API, you will need to instantiate a `CmvConfig` object.
...
```js {3}
import { CmvConfig } from '@mm-cmv/config';
...
```js {4-21}
import { CmvConfig, CoordinationLevel as CL } from '@mm-cmv/config';;
...
#### Parameters:
- `...cScopes` (variable number of `CmvConfigCoordinationScope`) - A variable number of coordination scope instances.
...
Function to enable configuration of multi-level coordination.
...
```js
import { CoordinationLevel as CL } from '@mm-cmv/config';;
```
