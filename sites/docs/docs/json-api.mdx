...
Intended to be used in situations where `<CoordinationProvider/>` is used as a controlled component (i.e., a parent component is the source of truth for the config prop and likely also uses the `onSpecChange` prop).
