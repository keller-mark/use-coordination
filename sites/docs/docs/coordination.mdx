...
mmCMV is a library for *coordinated multiple views*, meaning that components can be linked together on many properties.
The implementation of coordinated multiple views in mmCMV follows a coordination model proposed by Boukhelifa and Rodgers ([Information Visualization 2003](https://kar.kent.ac.uk/13874/1/cmvev.pdf)).
...
The following terms have been adapted from the information visualization literature and are used throughout the mmCMV documentation.
...
### Coordination Type
The property or parameter being coordinated, such as "spatial rotation" or "heatmap zoom" or "gene expression colormap". When using the `ZodCmvProvider`, mmCMV requires that coordination type values conform to a Zod schema, for instance a primitive number or a more complex array or object schema.
...
In mmCMV, the coordination space is stored in a serializable JSON format.
